                        ...
                        <div class="col-xl-12 col-lg-12 mb-4">
                            <div class="card shadow">
                                <div class="card-body">
                                    ...
                                    $datapb = site_url("datapb/$jenis_biaya/$id_transaksi/$id_pb");
                                    ?>
                                    <div class="row">
                                        ...
                                        <table class="table table-bordered mb-4">
                                            ...
                                            <form action="" method="POST" enctype="multipart/form-data">
                                                ...
                                                <input required class="form-control col-xl-4 col-lg-4 mt-3" type="date" name="tanggal" id="input_tanggal" value="<?php
                                                    foreach ($nomor1 as $nomor) {
                                                        echo(isset($tanggal1)) ? $tanggal1 : $nomor['tanggal'];
                                                    }
                                                ?>">
                                                <input name="id_valas" type="hidden" value="<?php
                                                    foreach ($nomor1 as $nomor) {
                                                        foreach ($valas1 as $key => $value) {
                                                            if ($nomor['id_pb'] == $value['id_pb']) {
                                                                $id_valas = $value['id_valas'];
                                                                $kode_valas = $value['kode_valas'];
                                                                $id_transaksi = session()->get('id_transaksi');

                                                                echo(isset($vala1)) ? $vala1 : $id_valas;
                                                            }
                                                        }
                                                    }
                                                ?>">
                                                <input name="kode_valas" type="hidden" value="<?php
                                                    foreach ($nomor1 as $nomor) {
                                                        foreach ($valas1 as $key => $value) {
                                                            if ($nomor['id_pb'] == $value['id_pb']) {
                                                                $id_valas = $value['id_valas'];
                                                                ...
                                                                echo(isset($vala)) ? $vala : $kode_valas;
                                                            }
                                                        }
                                                    }
                                                ?>">

                                                <input type="submit" class="btn btn-success mt-3" name="submit" value="Simpan Data" id="submit">
                                                <a href= "<?php echo $datapb ?>" class="btn btn-secondary mt-3"> Kembali</a>
                                            </form>
                                        </div>
                                    </div>
                                </div>
