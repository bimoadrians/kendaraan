...
<body id="page-top">
                            ...
                            <div class="card shadow">
                                <div class="card-body">
                                    <form action="" method="POST" enctype="multipart/form-data">
                                        ...
                                        <?php if ($role == 'admin' || $role == 'user') { ?>
                                            <?php if ($submit == 0) { ?>
                                                <!-- <a class="btn btn-primary mb-3" href="javascript:void(0)" data-toggle="modal" data-target="#nopb">Tambah Data Nomor PB</a> -->
                                                <a onclick="return confirm('Apakah data PB sudah lengkap dan benar?')"><input type="submit" class="btn btn-success mb-3" name="submit" value="Kirim Data PB ke Treasury" id="submit"></a>
                                            <?php  } else { ?>
                                        ...
                                        <?php } if ($role == 'treasury') { ?>
                                            <?php if ($submit < 2) { ?>
                                                    <?php if ($role == 'treasury' && $submit == 1) { ?>
                                                        <a onclick="return confirm('Apakah data PB perlu dilakukan revisi oleh User bagian?')"><input type="submit" class="btn btn-warning mb-3" name="submit" value="Revisi Data PB" id="submit"></a>
                                                    <?php } ?>
                                                    <?php if ($submit_pjum == 1 || $submit_pjum == 4) { ?>
                                                        <a class="btn btn-success mb-3" onclick="return confirm('Apakah data PB sudah lengkap dan benar?')" href="<?php echo $treasury ?>">Kirim Data PB ke GS</a>
                                                    <?php } ?>
                                                <?php  } else { ?>
                                        ...
                                        <?php } if ($role == 'gs') { ?>
                                            <?php if ($submit == 2) { ?>
                                                <a onclick="return confirm('Apakah data PB perlu dilakukan revisi oleh bagian Treasury?')"><input type="submit" class="btn btn-warning mb-3" name="submit" value="Revisi Data PB" id="submit"></a>
                                                <?php if ($submit_pjum == 4) { ?>
                                                    <a class="btn btn-success mb-3" onclick="return confirm('Apakah data PB sudah lengkap dan benar?')" href="<?php echo $gs ?>">Selesai</a>
                                                <?php } ?>
                                            <?php  } else { ?>
                                    ...
                                    <table id="myTable" class="table table-bordered mb-4">
                                        <thead>
                                            <tr>
                                                <th class="text-center" style="color: #5a5c69;">Nomor PB</th>
                                        ...
                                        <tbody>
                                            <?php foreach ($nomor as $nom) { 
                                                $id_pb = $nom['id_pb'];
                                                $link_pilih = site_url("datapb/$jenis_biaya2/$id_transaksi/$id_pb");
                                                $link_delete = site_url("listpb/$jenis_biaya2/$id_transaksi/?aksi=hapus&id_pb=$id_pb");
                                            ?>
                                                <tr>
                                                    <?php foreach ($valas as $key => $value) { ?>
                                                        <?php if($nom['id_pb'] == $value['id_pb']){ ?>
                                                            <?php if($role == 'admin' && $submit == 0 || $role == 'user' && $submit == 0 || $role == 'treasury' && $submit == 0 && $submit_pjum == 1 || $role == 'treasury' && $submit == 0 && $submit_pjum == 4){ ?>
                                                                <th class="text-center" scope="row" style="color: #5a5c69;">
                                                                    <a href="javascript:void(0)" style="color: #5a5c69" data-toggle="modal" data-target="#editnopb<?php echo $nom['id_pb']; echo $value['id_pb']; ?>"><abbr title="Klik untuk melakukan edit No PB" style="text-decoration:none;-webkit-text-decoration:none;text-decoration:none;cursor:pointer;border-bottom:0;-webkit-text-decoration-skip-ink:none;text-decoration-skip-ink:none"><?php echo $nom['nomor'];?></abbr></a>
                                                                </th>
                                                            <?php } else { ?>
                                                                <th class="text-center" scope="row" style="color: #5a5c69;">
                                                                    <?php echo $nom['nomor'];?>
                                                    ...
                                                    <?php foreach ($valas as $key => $value) { ?>
                                                        <?php if($nom['id_pb'] == $value['id_pb']){ ?>
                                                            <th class="text-center" scope="row" style="color: #5a5c69;"><?php echo $value['kode_valas'];?></th>
                                                        <?php } ?>
                                                    <?php } ?>

                                                    <th class="text-center" scope="row" style="color: #5a5c69;">
                                                        <a href="<?php echo $link_pilih?>" class="btn btn-success">Pilih</a>
                                                        <?php if ($role == 'admin' && $submit == 0 || $role == 'user' && $submit == 0 || $role == 'treasury' && $submit == 0 && $submit_pjum == 1 && $kirim_pb == 0 || $role == 'treasury' && $submit == 0 && $submit_pjum == 4 && $kirim_pb == 0) { ?>
                                                            <a href="<?php echo $link_delete?>" onclick="return confirm('Yakin akan menghapus data PB?')" class="btn btn-danger">Hapus</a>
                                                        <?php  } else { ?>
                                                                
                                                        <?php } ?>

                                                        <?php foreach ($kategori as $key => $value) { ?>
                                                            <?php if($value['id_pb'] == $nom['id_pb']) { ?>
    ...
    <!-- <div class="modal fade" id="nopb" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">No PB</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <input required type="text" autocomplete="off" class="form-control" id="nomor" name="nomor" placeholder="Masukkan no PB">
                    </div>
                    <div class="modal-footer">
    ...
    ?>
        <?php foreach ($valas as $key => $value) {
            if ($nom['id_pb'] == $value['id_pb']) {
                $id_valas = $value['id_valas'];
                $kode_valas = $value['kode_valas'];
            }
        ?>
            <div class="modal fade" id="editnopb<?php echo $nom['id_pb']; echo $value['id_pb']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Edit No PB</h5>
                            <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">×</span>
                            </button>
                        </div>
                        <form action="<?php echo site_url("editnopb/$id_pb/$id_transaksi/$jenis_biaya") ?>" method="post" enctype="multipart/form-data">
                            <div class="modal-body">
