...
class JenisBBMModel extends Model
{
    protected $table = "jenis_bbm";
    protected $primaryKey = "id_jenis_bbm";
    protected $allowedFields = ['jenis_bbm'];

    public function getData($parameter)
    {
        $builder = $this->table /*builder*/ ($this->table /*Model*/);

        $builder->select('nama_jenis_bbm');
        $builder->where('id_jenis_bbm=', $parameter);
        $query = $builder->get();
        return $query->getRowArray();
    }

    public function jenis_bbm_id($id_jenis_bbm)
    {
        $builder = $this->table($this->table);

        $builder -> where('id_jenis_bbm', $id_jenis_bbm);

        $query = $builder->get();
        return $query->getRowArray();
    }

    function delete_jenis_bbm($id_jenis_bbm){
        $builder= $this->table($this->table);
        $builder->where('id_jenis_bbm', $id_jenis_bbm);
        if($builder->delete()){
            return true;
